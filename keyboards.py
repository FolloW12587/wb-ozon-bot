...
def create_remove_and_edit_sale_kb(
    user_id: int,
    product_id: str,
    marker: Literal["wb", "ozon"],
    job_id: str,
...
def add_graphic_btn(_kb: InlineKeyboardBuilder, user_id: int, product_id: int):
    _callback_data = f"graphic_{user_id}_{product_id}"

    _kb.row(
        types.InlineKeyboardButton(
            text="График изменения цен", callback_data=_callback_data
        )
    )
...
def add_pagination_btn(_kb: InlineKeyboardBuilder, product_dict: dict):
    ...
    match kb_init:
        case "start":
            _kb.row(
                types.InlineKeyboardButton(
                    text=f"{current_page}/{pages}", callback_data="pagination_page"
                )
            )
            _kb.add(types.InlineKeyboardButton(text="▶", callback_data="page_next"))
        case "mid":
            _kb.row(types.InlineKeyboardButton(text="◀", callback_data="page_prev"))
            _kb.add(
                types.InlineKeyboardButton(
                    text=f"{current_page}/{pages}", callback_data="pagination_page"
                )
...
def create_or_add_return_to_product_list_btn(_kb: InlineKeyboardBuilder = None):
    ...
    _kb.row(
        types.InlineKeyboardButton(
            text="Вернуться к списку товаров", callback_data="return_to_product_list"
        )
    )
...
def create_pagination_page_kb(product_dict: dict):
    ...
    for page_num in range(1, pages + 1):
        ...
        _kb.row(
            types.InlineKeyboardButton(
                text=_text, callback_data=f"go_to_page_{page_num}"
            )
        )
