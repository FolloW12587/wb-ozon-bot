...
async def check_user_last_message_time(
    user_id: int,
    now_time: datetime,
    message_text: str,
    ...
    async with lock:
        ...
        if last_action_time := user_data.get("last_action_time"):
            ...
            _last_action_time = datetime.fromtimestamp(last_action_time).astimezone(
                moscow_tz
            )
            ...
            if time_delta >= datetime.fromtimestamp(last_action_time).astimezone(
                moscow_tz
            ):
                # first message
                #
                ...
                if message_text.isdigit():
                    ...
                    await save_product(user_data, session, scheduler)
                ...
                if message_text.isdigit():
                    ...
                    await add_procent_to_product(user_data, session, _percent)
                    ...
                    await save_product(user_data, session, scheduler, percent=percent)
...
async def add_procent_to_product(user_data: dict, session: AsyncSession, percent: str):
    ...
    ## WB
        ...
        if not res:
            await bot.send_message(
                chat_id=msg[0], text="Не получилось найти пункт выдачи"
            )
            return
...
async def clear_state_and_redirect_to_start(
    message: types.Message | types.CallbackQuery, state: FSMContext, bot: Bot
):
    await state.clear()
...
async def add_popular_product_to_db(_redis_pool: ArqRedis, file_path: str, reply_chat_id: int):
    ...
    await msg.edit_text("Все строки успешно добавлены в очередь, ожидайте появления в бд")
    os.remove(file_path)
